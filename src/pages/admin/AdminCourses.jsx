...
export default function AdminCourses() {
  ...
  async function refreshCourses() {
    ...
    setCourses(snap.val() ? Object.entries(snap.val()).map(([id, value]) => ({ id, ...value })) : [])
  }
  ...
  async function saveCourse(e) {
    ...
    try {
      ...
      const hasScheduleInput = Boolean(form.days.trim() || form.startTime.trim() || form.endTime.trim())
      const slots = hasScheduleInput
        ? expandSlots(form.days, form.startTime, form.endTime, form.room, 'Lecture')
        : []

      if (hasScheduleInput) {
        const { errors } = validateSchedule(slots)
        if (errors.length > 0) {
    ...
    <div className="max-w-3xl space-y-8">
      <div>
        ...
        <form onSubmit={saveCourse} className="grid grid-cols-2 gap-3">
          ...
          {message && <p className="col-span-2 text-sm text-emerald-400">{message}</p>}
        </form>
      </div>
